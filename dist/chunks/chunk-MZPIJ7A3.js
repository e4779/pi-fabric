  ...
  async execute(code, hostCall, options) {
    ...
    if (missingKeys.length > 0) {
      ...
      return {
        ...
        error: "Pre-execution check: " + missingKeys.map((key) => "\u03C0." + key).join(", ") + (missingKeys.length === 1 ? " is referenced in code but its key is missing from the payloads parameter" : " are referenced in code but their keys are missing from the payloads parameter") + (provided.length ? " (provided: " + provided.join(", ") + ")" : " (none provided)") + ". Add payloads: { " + missingKeys.join(": '...', ") + ": '...' } to the fabric_exec arguments, then reference the value as \u03C0.<key>. For large or quote-heavy content, keep it in top-level payloads and reference \u03C0.<key> instead of escaping it inside code."
      };
    }
...
//# sourceMappingURL=chunk-MZPIJ7A3.js.map
